...
| 2 | line覆盖率未达100% | nid更新逻辑中部分情况不可达，鉴定为死代码 | 更新nid更新逻辑 |
...
| --- | ---: | ---: | --- |
| Line | 100% | 100% | 数值达标 |
| Condition | 99.86% | 100% | 未达标，缺口 0.14%；剩下为不可达组合，`代码覆盖率排除说明` 分析结构性不可达组合 |
| Branch | 100% | 100% | 数值达标 |
| Toggle（ports only） | 98.67% | 100% | 未达标，缺口 1.33%；剩下为无用信号，`代码覆盖率排除说明` 分析无用信号 |
